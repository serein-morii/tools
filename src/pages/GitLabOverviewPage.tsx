...
import { RefreshCw, BarChart3, Users, GitCommit, TrendingUp, TrendingDown, MinusCircle, Inbox, GitBranch, ArrowUpDown, ArrowUp, ArrowDown, ShieldAlert, Bug, Zap, Loader2, GitMerge, ExternalLink, Shield, CheckCircle2, XCircle, Clock3, AlertTriangle, Activity, FlaskConical, Settings } from "lucide-react";
    ...
    <div className="space-y-2">
      <div className="grid grid-cols-4 gap-3">
        {cards.map((card) => (
          <div
            key={card.id}
            className="stat-card-compact cursor-pointer"
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate mt-0.5">{card.label}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {expandedCard && current && (
        <div className="rounded-xl border bg-card/50 px-4 py-3 animate-in fade-in slide-in-from-top-1 duration-150">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 max-h-48 overflow-y-auto">
                {currentProjects.map((p) => (
                  <div key={p.project_id} className="flex items-center gap-2 rounded-lg bg-muted/30 px-3 py-2 transition-colors hover:bg-muted/50">
                    <span className="text-xs truncate flex-1 font-medium" title={p.project_name}>{p.project_name.split("/").pop()}</span>
                    {p.has_test && <span className="shrink-0 text-xs text-emerald-600 font-medium tabular-nums">{p.commits}c</span>}
                    {!p.has_test && <span className="shrink-0 text-xs text-muted-foreground">✗</span>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {expandedCard === "commits" && (
            <div className="space-y-3">
              <div className="flex items-center gap-6 text-xs text-muted-foreground">
                <span>总提交: <strong className="text-foreground tabular-nums">{current.total_commits}</strong></span>
                <span>+行: <strong className="text-emerald-600 tabular-nums">{current.total_lines_added.toLocaleString()}</strong></span>
                <span>-行: <strong className="text-destructive tabular-nums">{current.total_lines_removed.toLocaleString()}</strong></span>
              </div>
              {currentDevStats.length > 0 && (
                <div className="space-y-1.5 max-h-36 overflow-y-auto">
                  <div className="text-xs text-muted-foreground font-medium">开发者贡献明细</div>
                  {currentDevStats.slice(0, 10).map((dev, i) => (
                    <div key={dev.name} className="flex items-center gap-3 text-xs bg-muted/20 rounded-lg px-3 py-2 transition-colors hover:bg-muted/30">
                      <span className={cn("w-5 text-center shrink-0 flex items-center justify-center rounded-full text-xs font-bold",
                      ...
                      <span className="text-emerald-600 tabular-nums">+{dev.lines_added.toLocaleString()}</span>
                      <span className="text-destructive tabular-nums">-{dev.lines_removed.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                {currentContributorList.map((name) => (
                  <span key={name} className="rounded-lg bg-muted/50 px-3 py-1.5 text-xs font-medium transition-colors hover:bg-muted">{name}</span>
                ))}
              </div>
            </div>
          )}
          {expandedCard === "coverage" && (
            <div className="space-y-3">
              <div className="flex items-center gap-6 text-xs text-muted-foreground">
                <span>有单测: <strong className="text-emerald-600 tabular-nums">{current.test_projects}</strong></span>
                <span>无单测: <strong className="text-destructive tabular-nums">{current.total_projects - current.test_projects}</strong></span>
                <span>覆盖率: <strong className="text-foreground tabular-nums">{current.total_projects > 0 ? Math.round((current.test_projects / current.total_projects) * 100) : 0}%</strong></span>
              </div>
              ...
              <div className="space-y-1.5 max-h-36 overflow-y-auto">
                <div className="text-xs text-muted-foreground font-medium">无单测项目</div>
                {currentProjects.filter(p => !p.has_test).slice(0, 10).map((p) => (
                  <div key={p.project_id} className="flex items-center text-xs text-muted-foreground bg-muted/20 rounded-lg px-3 py-1.5 transition-colors hover:bg-muted/30">
                    <span className="flex-1 truncate font-medium">{p.project_name.split("/").pop()}</span>
                    <span className="tabular-nums">{p.commits} commits</span>
                  </div>
                ))}
                {currentProjects.filter(p => !p.has_test).length > 10 && (
                  <div className="text-xs text-muted-foreground text-center">...还有 {currentProjects.filter(p => !p.has_test).length - 10} 个</div>
                )}
              </div>
            </div>
          )}
...
export function GitLabOverviewPage() {
  ...
  const { data: isConfigured } = useGitLabConfigured();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button size="sm" onClick={handleScan} disabled={triggerScan.isPending}>
            <RefreshCw className={`mr-1.5 h-4 w-4 ${triggerScan.isPending ? "animate-spin" : ""}`} />
